          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-5">
            {items.map((item) => (
              <div key={item.id} className="flex bg-white rounded-lg ">
                {/* 이미지 */}
                <img src={item.img} alt={item.title} className="w-24 h-24 rounded-md object-cover" />
                ...
                <div className="ml-3 flex-1">
                  ...
                  <div className="flex gap-1 mb-1">
                    {item.tags.map((tag, index) => (
                      <span key={index} className="bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-md">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  {/* 제목 */}
                  <h3 className="text-sm font-semibold truncate w-40">{item.title}</h3>
